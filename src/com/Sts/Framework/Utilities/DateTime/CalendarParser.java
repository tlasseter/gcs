...
class ParserState
{
            ...
            throws CalendarParserException
    {
        ...
        if(timePostMeridian)
        {
            // in case both PM and 24-hour time specified, e.g., 17:00 PM, don't add 12 
            if(val < 12)
                tmpHour = val + 12;
            else
                tmpHour = val;
...
public class CalendarParser
{
    ...
    public static final int DD_MM_YY =
            YEAR_AFTER_DAY | YEAR_AFTER_DAY | MONTH_AFTER_DAY;
    ...
    public static final int[] dateOrder = { DD_MM_YY, MM_DD_YY, MM_YY_DD, DD_YY_MM, YY_DD_MM, YY_MM_DD };
    public static final String[] dateOrderStrings = { "DD_MM_YY", "MM_DD_YY", "MM_YY_DD", "DD_YY_MM", "YY_DD_MM", "YY_MM_DD" };
    ...
    public static final int getOrderFromString(String orderString)
    {
        for(int i=0; i<dateOrderStrings.length; i++)
        {
              if(orderString.equalsIgnoreCase(dateOrderStrings[i]))
                  return dateOrder[i];
        }
        return -1;
    }

    public static final String getStringFromDateOrder(int dOrder)
    {
        for(int i=0; i<dateOrder.length; i++)
        {
              if(dateOrder[i] == dOrder)
                  return dateOrderStrings[i];
        }
        return null;
    }
	...
	static public long getLongFromDateTimeString(String dateTime)
	{
		return getLongFromDateTimeString(dateTime, CalendarParser.DD_MM_YY);
	}

	static public long getLongFromDateTimeString(String dateTime, int dateOrder)
	{
		try
		{
			return CalendarParser.parse(dateTime, dateOrder).getTimeInMillis();
		}
		catch(Exception e)
		{
			StsException.systemError(CalendarParser.class, "getLongFromDateTimeString", "Failed to parse dateTime: " + dateTime);
			return -1L;
		}
	}

    static public String getDateStringFromLong(long time, SimpleDateFormat dateFormat)
    {
        return dateFormat.format(new Date(time));
    }

    static public String getTimeStringFromLong(long time, SimpleDateFormat timeFormat)
    {
        return timeFormat.format(new Date(time));
    }
	
	static public String getDateTimeStringFromLong(long time, SimpleDateFormat dateTimeFormat)
	{
		return dateTimeFormat.format(new Date(time));
	}
	
	// TODO should be a better way to use the parser itself
	static public String getDateFromString(String dateTime)
	{
    ...
    {
        int dateOrder = DD_MM_YY;
        String dateToken = "Jan 1 1960";
        String timeToken = "18:26:45 AM";
        String sTime = dateToken + " " + timeToken;
        System.out.print("Input date: " + sTime);
        try
        {
            Calendar cal = CalendarParser.parse(sTime, dateOrder, true);
            System.out.println(" Output date: " + CalendarParser.prettyString(cal));
        }
        catch(Exception e)
        {
            StsException.outputWarningException(CalendarParser.class, "main", e);
        }
    }
}
