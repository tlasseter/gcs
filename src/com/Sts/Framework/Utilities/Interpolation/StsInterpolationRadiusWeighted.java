 ...
 */
public class StsInterpolationRadiusWeighted
{
    private float[][] pointsZ;
    private byte[][] pointsNull;
    private int rowMin, rowMax, colMin, colMax;
//    private byte interpolatedNullType = StsSurface.NULL_GAP_FILLED;
...
//    private int maxSpiral;
    ...
    private StsInterpolationRadiusWeighted(float[][] pointsZ, byte[][] pointsNull,
                int rowMin, int rowMax, int colMin, int colMax)
    {
        ...
        this.colMin = colMin;
        this.colMax = colMax;
//        this.interpolatedNullType = interpolatedNullType;
    }

    static public StsInterpolationRadiusWeighted constructor(StsSurface surface)
    {
        return new StsInterpolationRadiusWeighted(surface.getPointsZ(), surface.getPointsNull(),
            surface.getRowMin(), surface.getRowMax(), surface.getColMin(), surface.getColMax());
    }
...
//            foundMaxNPoints = false;
        ...
        {
            StsException.outputException("StsInterpolationRadiusWeighted.doInterpolate() failed." +
...
//                    weightZ += ((Point)points.get(n)).z*dwt;
            ...
            if(minDistance <= maxInterpolationDistance)
                setPointNull(iCenter, jCenter, StsParameters.SURF_GAP_FILLED);
            else
                setPointNull(iCenter, jCenter, StsParameters.SURF_BOUNDARY);
//          pointsNull[iCenter][jCenter] = interpolatedNullType;
        }
        catch(Exception e)
        {
            StsException.outputException ("StsInterpolationRadiusWeighted.computeZ() failed.",
                e, StsException.WARNING);
        }
    }
    ...
    public final void setPointNull(int row, int col, byte nullType)
    {
        ...
        if (Main.debugPoint && row == Main.debugPointRow && col == Main.debugPointCol)
        {
            ...
            StsException.systemDebug(this, "setPointNull",
                "pointsNull[" + row + "][" + col + "] changed to " + StsParameters.getSurfacePointTypeName(nullType) +
