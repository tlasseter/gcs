...
public class StsParameterFileNew
{
	...
	static public void writeObjectFields(StsAsciiFile asciiFile, Object obj, Class superClass) throws StsException
	{
		writeObjectFields(asciiFile, obj.getClass(), superClass, obj);
	}
	...
	static public void writeObjectFields(StsAsciiFile file, Class subClass, Class superClass, Object object)
	{
		String prefix = "";
	...
	static public void writeObjectFields(StsAsciiFile file, Class subClass, Class superClass, Object object, String prefix)
	{
		try
		{
			...
			for(int n = 0; n < nFields; n++)
			{
				...
				{
					int dimension = 0;
					Class componentType = fieldObject.getClass();
					while (componentType.isArray())
					{
						dimension++;
						componentType = componentType.getComponentType();
					}
					file.writeString(Integer.toString(dimension));
					file.endLine();
					writeArray(file, componentType, fieldObject, prefix, dimension);
				}
				else
				{
		...
		{
			StsException.outputWarningException(StsParameterFileNew.class, "writeStsAsciiHeader", e);
		}
	}

	static public boolean writeArray(StsAsciiFile file, Class arrayType, Object fieldObject, String prefix, int dimension)
	{
		try
		{
			if(dimension > 1)
			{
				...
				for(int i = 0; i < nObjects; i++)
					writeArray(file, arrayType, objects[i], prefix + "   ", dimension - 1);
				return true;
			}
		...
		{
			StsException.outputWarningException(StsParameterFileNew.class, "writeArray", e);
			return false;
		}
	}
...
//                        " Missing value for required parameter '" + fields[i].getName() + "'");
	...
	static public boolean initialReadObjectFields(String pathname, Object object, Class subClass, Class superClass) throws StsException
	{
		try
		{
			...
			try
			{
				...
				return StsParameterFileNew.readObjectFields(file, object, subClass, superClass);
			}
			catch(NumberFormatException e)
			{
				return false;
				// return StsParameterFileOld.readObjectFields(file, object, subClass, superClass);
			}
		}
		catch(IOException e)
		{
			StsException.outputWarningException(StsParameterFileNew.class, "initialiReadObjectFields", e);
			return false;
		}
	}
	 ...
	 */
	static public boolean readObjectFields(StsAbstractFile file, Object object, Class subClass, Class superClass) throws IOException, StsException, NumberFormatException
	{
	...
	static public boolean readObjectFields(StsAbstractFile file, Object object, TreeMap fieldsTreeMap) throws IOException, StsException, NumberFormatException
	{
		boolean isMissingFields = false;
		missingFields = new ArrayList();
		String line;
		...
		{
			StsException.outputWarningException(StsParameterFileNew.class, "readObjectFields", e);
			return false;
		}
		finally
		{
...
//            else break;
	...
	static public void main(String[] args)
	{
		try
		{
			...
			String[][] tableValues = new String[][]{{"A1", "B1", "C1"}, {"A2", "B2", "C2"}, {"A3", "B3", "C3"}, {"A4", "B4", "C4"}};
			DataTable table = new DataTable(columnNames, tableValues);
			StsParameterFileNew.writeObjectFields(args[0], table);
			table = new DataTable();
			StsParameterFileNew.readObjectFields(args[0], table);
			// table.print();
		/*
			Strings3D strings3d = new Strings3D();
			StsParameterFile.writeObjectFields(args[0], strings3d);
        */
			/*
						String string = "    12";
...
class Strings3D
{
	...
	Strings3D()
	{
		...
		int nCols = 4;
		int nSlices = 5;
		String[] rows = new String[] { "rA", "rB", "rC", "rD", "rE" };
		String[] cols = new String[] { "cA", "cB", "cC", "cD", "cE" };
		String[] slices = new String[] { "sA", "sB", "sC", "sD", "sE" };
		strings3D = new String[nRows][nCols][nSlices];
		for(int r = 0; r < 3; r++)
			for(int c = 0; c < 4; c++)
				for(int s = 0; s < 5; s++)
					strings3D[r][c][s] = rows[r]+cols[c]+slices[s];
	}
}
...
class DataTable
{
	...
	DataTable(String[] columnNames, String[][] tableValues)
	{
