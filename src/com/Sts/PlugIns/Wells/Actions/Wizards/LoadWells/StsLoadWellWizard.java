...
public class StsLoadWellWizard extends StsLoadWizard
{
    public StsLoadWellWizard(StsActionManager actionManager)
    {
        super(actionManager, 600, 700);
    }

	public StsLoadWellWizard(StsActionManager actionManager, int width, int height)
    {
	...
	{
		return new StsSourceSelectStep(this, StsWell.groupName, null, StsLoaderDataDirectorySets.wellOutputTypes );
	}
	...
	public void initializeSelectedFiles()
	{
		...
		for(StsAbstractFile file : files)
		{
			...
			subFileGroupsFilter.setName(name);
			Path directoryPath = file.getPath().getParent();
			String[] subFilenames = directoryPath.toFile().list(subFileGroupsFilter);
			int nSubFiles = subFilenames.length;
			StsAbstractFile[] subFiles = new StsAbstractFile[subFilenames.length];
			for(int n = 0; n < nSubFiles; n++)
			{
				Path subFilePath = directoryPath.resolve(Paths.get(subFilenames[n]));
				subFiles[n] = StsFile.constructor(subFilePath);
			}
			file.setSubFiles(subFiles);
		}
	}

	public void initializeSelectedFilesX()
	{
	...
	public void next()
	{
		if(currentStep == sourceSelectStep)
		{
			isSourceData = false;
			deleteStsData = false;
			//inputDirectory = sourceSelectStep.panel.getInputDirectory();
			dataDirectory = sourceSelectStep.panel.getDataDirectory();
			initializeNamesAndFilenameFilter();
			fileset = StsFileSet.constructor(dataDirectory, filenameFilter);
		...
		(
			new Runnable()
			{
				public void run()
				{
					...
					actionManager.startAction(StsLoadWellWizard.class);
